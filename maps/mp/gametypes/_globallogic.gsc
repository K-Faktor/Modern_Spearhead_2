...
init()
{
	...
	// Removed by Newfie to eliminate forcing of iwd filename
	...
	// Initialize variables used by Modern Spearhead 2
	ms2\_registerdvars::init();
...
SetupCallbacks()
{
	...
	level.onSpawnFtagPlayer = ::blank;
	level.onStartFtagGame = ::blank;
	level.onPlayerFrozen = ::blank;
...
default_onForfeit( team )
{
	...
	// if only 1 player on server do not forfeit
...
matchshownowlive()
{
	...
	game["scrim"]["status"] = &"MS2_LIVEMATCH";
...
menuAutoAssign()
{
	...
	if ( isdefined ( game["scrim"]["islive"] ) && game["scrim"]["islive"] && level.scr_disable_match_join ) {
		...
		if ( isdefined( game["scrim"]["names"] ) && issubstr( game["scrim"]["names"], self.name ) )
			ismatchplayer = true;
		if ( !ismatchplayer ) {
			self iprintlnbold(&"MS2_HAVENOPERMISSION");
			return;
		}
	}
...
menuClass( response )
{
	...
	if ( self.sessionstate == "playing" )
	{
		...
		if ( ( ( level.inGracePeriod || level.inStrategyPeriod ) && !self.hasDoneCombat && ( level.gametype != "ass" || !isDefined( self.isVIP ) || !self.isVIP ) ) || ( level.gametype == "ftag" && self.freezeTag["frozen"] ) || isDefined( self.frozen ) && self.frozen )
		{
...
Callback_StartGameType()
{
	if ( isdefined( level.scr_gameplay_ftag ) && level.scr_gameplay_ftag )
		level thread ms2\_ftagplay::init();
	...
	if ( !isDefined( game["gamestarted"] ) )
	{
		...
		game["strings"]["match__is_live"] = &"MS2_AACP_MATCH_LIVE";
		...
		game["scrim"]["status"] = &"MS2_PREMATCH";
...
Callback_PlayerDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	// Do not process hurt damage if player is frozen
	if ( isdefined( self.frozen ) && self.frozen ) 
		return;

	// Do not process hurt damage if player earned special powers
	if ( isDefined( self.invincible ) && self.invincible ) 
		return;
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	// Check if this was a Sticky Nade explosion
	if( isDefined( self.hasstickynade ) && self.hasstickynade )
	...
	if ( level.scr_show_obituaries != 0 && ( !isDefined( self.isBeingAnnoyed ) || !self.isBeingAnnoyed ) ) {
	...
	if ( !isDefined( self.switching_teams ) )
		self thread [[level.onPlayerFrozen]](eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration);

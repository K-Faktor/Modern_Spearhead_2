...
onGameEnded()
{
	...
	level.eogBest["melt"]["name"] = "";
	level.eogBest["melt"]["value"] = 0;	

	level.eogBest["meltstreak"]["name"] = "";
	level.eogBest["meltstreak"]["value"] = 0;
	...
	for ( index = 0; index < level.players.size; index++ )
	{
		...
		if ( isDefined( player ) && isDefined( player.pers["stats"] ) ) {	
			...
			player checkStatItem( player.pers["stats"]["misc"]["melt"], "melt" );
			player checkStatItem( player.pers["stats"]["misc"]["meltstreak"], "meltstreak" );
		}
	}
	...
	for ( index = 0; index < level.players.size; index++ )
	{
		...
		if ( isDefined( player ) ) {
			...
			player setClientDvars(
				...
				"gs_mln", level.eogBest["melt"]["name"],
				"gs_ml", level.eogBest["melt"]["value"],
				"gs_mlsn", level.eogBest["meltstreak"]["name"],
				"gs_mls", level.eogBest["meltstreak"]["value"]
			);			
		}
	}	
}
...
onPlayerConnected()
{
	...
	if ( !isDefined( self.pers["stats"] ) ) {
		...
		self setClientDvars(
			...
			"ps_ml", 0,												// melts
			"ps_mls", 0,												// Current meltstreak
			"ps_mlsh", 0												// Highest meltstreak
		);
		self setClientDvars(
			"ps_h", 0,												// Headshots
		...
		self.pers["stats"]["misc"]["meltstreak"] = 0;
		self.pers["stats"]["misc"]["meltconsecutive"] = 0;		
		self.pers["stats"]["misc"]["melt"] = 0;		
	}	
}
...
onPlayerMelted()
{
	...
	for(;;)
	{
		...
		if ( isDefined( self.hasSpecialPowers ) && self.hasSpecialPowers && isDefined( level.scr_disable_power_streaks ) && level.scr_disable_power_streaks )
			continue;

		self.pers["stats"]["misc"]["meltconsecutive"] += 1;		
		self.pers["stats"]["misc"]["melt"] += 1;

		if ( self.pers["stats"]["misc"]["meltconsecutive"] > self.pers["stats"]["misc"]["meltstreak"] )
			self.pers["stats"]["misc"]["meltstreak"] = self.pers["stats"]["misc"]["meltconsecutive"];

		self setClientDvars(
			"ps_ml", self.pers["stats"]["misc"]["melt"],
			"ps_mls", self.pers["stats"]["misc"]["meltconsecutive"],
			"ps_mlsh", self.pers["stats"]["misc"]["meltstreak"]
		);
	}
}
...
onPlayerKilled()
{
	...
	for (;;) {
		...
		if ( sMeansOfDeath == "MOD_SUICIDE" && isDefined( self.switching_teams ) && self.switching_teams )
			continue;

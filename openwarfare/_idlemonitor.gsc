	...
	for (;;)
	{
		...
		if ( isDefined( self.pers["team"] ) && self.pers["team"] != "spectator" ) {
			...
			if ( level.scr_idle_switch_spectator > 0 ) {
				...
				if ( isAlive( self ) && ( self getVelocity() != 0 || self attackButtonPressed() || self fragButtonPressed() || self meleeButtonPressed() || self secondaryOffhandButtonPressed() || self useButtonPressed() || ( !isDefined( self.idlePlayerAngles ) || self.idlePlayerAngles != playerAngles ) || ( level.gametype == "ftag" && self.freezeTag["frozen"] ) || isDefined( self.frozen ) && self.frozen ) ) {
					self.lastActivity = gettime();
					self.idleWarned = false;
					self destroyIdleTimer();

...
init()
{
	...
	// Get how often players can vote
	level.scr_amvs_vote_mapnumber = getdvard( "scr_amvs_vote_mapnumber", "int", 0, 0, 100 );
	level.scr_amvs_vote_mapagain = getdvard( "scr_amvs_vote_mapagain", "int", 0, 0, 100 );

	level thread addNewEvent( "onPlayerConnected", ::onPlayerConnected );
}


addMaptoVoteDisAllowed( mapname )
{
	if ( !level.scr_amvs_vote_mapagain )
		return;

	// Get and Save the current game number
	level.recentgames = getdvard( "scr_amvs_recentgames", "string", "" );

	// Load recent maps voted
	level.recentlist = strtok( toLower( level.recentgames ), ";" );

	// If we don't have any maps voted then add and return
	if ( level.recentlist.size == 0 ) {
		level.recentgames = mapname + ";";
		setDvar( "scr_amvs_recentgames", level.recentgames );

		// Send the line to the server log
		logPrint( "MVS: Adding map " + mapname + " to recent map list of " + level.recentgames + ". \n" );
		return;
	}

	// Add this map to the list in last pos
	level.recentlist[level.recentlist.size] = mapname;

	// If we have all the recent amount of maps, delete the first one and make room
	if ( level.recentlist.size >= level.scr_amvs_vote_mapagain ) {
		for ( i = 0; i < level.recentlist.size - 1; i++ ) {
			level.recentlist[i] = level.recentlist[i + 1];
		}
	}
	...
	for ( i = 0; i < level.recentlist.size - 1; i++ ) {
		level.recentgames += level.recentlist[i] + ";";
	}
...
isMapVoteAllowed( mapname )
{
	...
	// If we don't have any maps voted then return ok
	if ( level.recentlist.size == 0 )
		return true;

	// Check if current game has been voted recently
	for ( i = 0; i < level.recentlist.size; i++ ) {
		if ( issubstr( mapname, level.recentlist[i] ) )
			return false;
	}		

	// If we made it hear then map is not in list
	return true;
}
...
mapVoting_Intermission()
{
	...
	if ( level.scr_amvs_vote_mapnumber )
	{
		// Get and Save the current game number
		level.curgamenumber = getdvard( "scr_amvs_curgamenumber", "int", 0, 0, 100 );
		if( !isDefined( level.curgamenumber ) )
			level.curgamenumber = 0;

		// Increase the game number and save dvar
		level.curgamenumber++;
		setDvar( "scr_amvs_curgamenumber", level.curgamenumber );

		// Check if we can allow players to vote
		if ( level.curgamenumber != level.scr_amvs_vote_mapnumber )
			return;

		// Reset the game to 0 and let players vote
		level.curgamenumber  = 0;
		setDvar( "scr_amvs_curgamenumber", level.curgamenumber );
	}
	...
	// Save the next gametype in the rotation
	if ( isDefined ( level.nextMapInfo ) && isDefined ( level.nextMapInfo["gametype"] ) )
		level.mapVoteNextGametype = level.nextMapInfo["gametype"];
	else
		level.mapVoteNextGametype = "";

	// Save the next map in the rotation
	if ( isDefined ( level.nextMapInfo ) && isDefined ( level.nextMapInfo["mapname"] ) )
		level.mapVoteNextMap = level.nextMapInfo["mapname"];
	else
		level.mapVoteNextMap = "";
		...
		for (type=0; type < level.scr_amvs_gametypes.size; type++ ) {
			...
			if ( thisGameType.size == 0 ) {
				...
				for (;;) {
					...
					for ( index=0; index < thisLine.size; index++ ) {
						if ( level.scr_amvs_can_repeat_map == 1 || thisLine[index] != level.script ) {
							if ( isMapVoteAllowed( thisLine[index] ) )
								thisGameType[thisGameType.size] = thisLine[index];
						}
					}
					// Process next line
		...
		for ( mg=0; mg < mgCombinations.size; mg++ ) {
			...
			if ( !isDefined( auxVar[ mgCombinations[mg]["gametype"] ][ mgCombinations[mg]["mapname"] ] ) ) {
				...
				if ( level.scr_amvs_can_repeat_map == 1 || mgCombinations[mg]["mapname"] != level.script ) {
					if ( isMapVoteAllowed( mgCombinations[mg]["mapname"] ) )
...
determineMapWinner()
{
	...
	// Add map to recent list
	addMaptoVoteDisAllowed( level.amvsWinnerMap );

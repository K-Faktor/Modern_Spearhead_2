...
monitorPlayerCamping()
{
	...
	for (;;)
	{
		...
		while ( distanceMoved < level.scr_anti_camping_distance && campingTime > openwarfare\_timer::getTimePassed() ) {
			...
			wait (0.25);
			if ( ( isDefined( self.interacting_with_objective ) && self.interacting_with_objective ) || ( isDefined( self.isVIP ) && self.isVIP ) || ( isDefined( self.isDefusing ) && self.isDefusing ) || ( isDefined( self.isPlanting ) && self.isPlanting ) || ( level.gametype == "ftag" && self.freezeTag["frozen"] ) || ( isDefined(self.frozen) && self.frozen ) )
				break;
		}

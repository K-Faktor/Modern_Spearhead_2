...
waitForKill()
{
	...
	if ( self.pers["obituarymsg"] )
		return;

	self.pers["obituarymsg"] = true;
	...
	if ( !isdefined(level.scr_gameplay_ftag) || !level.scr_gameplay_ftag ) {
		...
		// Display the extended information to the victim
		self iprintlnbold( &"MS2_EXTENDED_OBITUARY_KILLEDBY_WEAPON", eAttacker.name, sWeapon );
		self iprintlnbold( &"OW_EXTENDED_OBITUARY_LOCATION_DISTANCE", sHitLoc, distToShow );

		// Check if we should show the extended obituary to all the players or just the attacker
		if ( level.scr_show_ext_obituaries == 2 ) {
			eAttacker iprintln( &"MS2_EXTENDED_OBITUARY", eAttacker.name, self.name, sHitLoc, sWeapon, distToShow );

		} else if ( level.scr_show_ext_obituaries == 3 ) {
			iprintln( &"MS2_EXTENDED_OBITUARY", eAttacker.name, self.name, sHitLoc, sWeapon, distToShow );		
		}
	}

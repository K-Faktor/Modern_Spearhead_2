...
CountdownMonitor()
{
  ...
  if ( isdefined ( level.ftagactive ) && level.ftagactive && isdefined( level.inOvertime ) && level.inOvertime )
    timeLeft = level.OTtimeLeft;
  else
  ...
  while ( game["state"] == "playing" )
      ...
      if ( !level.timerStopped && level.timeLimit )
	{
	  ...
	  if (( level.soundFlags & level.CDFLAG_FINAL ) && ( timeLeft <= 10 && !( playedAlready & level.CDFLAG_FINAL )))
	    {
	      if ( isdefined ( level.ftagactive ) && level.ftagactive && ( !isdefined( level.inOvertime ) || isdefined( level.inOvertime ) && !level.inOvertime ) )
		playSoundOnPlayers( "CDFinalOT" );
	      else

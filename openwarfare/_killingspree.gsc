...
init()
{
	...
	level.scr_boom_headshot_sound = getdvarx( "scr_boom_headshot_sound", "int", 0, 0, 1 );
...
onPlayerKillStreak()
{
	...
	for(;;)
	{
		...
		// Check if we need to play boomheadshot sound
		if ( level.scr_boom_headshot_sound == 1 && !playedSound && sMeansOfDeath == "MOD_HEAD_SHOT" ) {
			playedSound = true;
			self playLocalSound( "boomheadshot" );
		}
	}
}